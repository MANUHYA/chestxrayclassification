...
st.markdown(f'''
<h1 align='center'>ChestXray Classification System</h1>
...
if btn and user_image is not None:
	...
	initial_probs_list=model_img[0].probs
	final_probs_list=initial_probs_list.data.tolist()
	final_val=np.argmax(final_probs_list)
	final_pred_val=names_dict[final_val]
	st.image(user_image,use_column_width=True)
	st.markdown(f'''
    <h3 align='center'>ChestXray Classified Class: {final_pred_val}</h3>''',unsafe_allow_html=True)
	st.balloons()
